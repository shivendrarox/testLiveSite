      ...
      itemListElements={[
        {
          position: 1,
          name: 'Home',
          item: 'https://mailmodo.com/',
        },
        {
          position: 2,
          name: 'Email-templates',
          item: 'https://mailmodo.com/email-templates/',
        },
        {
          position: 3,
          name: this.props.pageTitle,
          item: 'https://mailmodo.com/email-templates/'+this.props.slug,
        },
      ]}
    />
                        ...
                        <li className={styles.breadcrumb_item} key={index}>
                                {//<p disabled={item.active ? true : false}>{(index == 2)?this.props.blogTitle:item.text}</p>
                                 ...
                                 <Link passHref href={item.link}>
                                    {(index == breadcrumbs.length-1)?<a title={this.props.pageTitle} disabled >{item.text}</a>:<a title={item.text} disabled={item.active ? true : false}>{item.text}</a>}
                                </Link>
                            <img hidden={(index == breadcrumbs.length-1)?true:false} alt="breadcrumb arrow" title="breadcrumb arrow icon" style={{opacity:"0.4"}} src="/static/images/icon/breadcrumb-arrow-templates.svg"/>
                        </li>
                    ))}
